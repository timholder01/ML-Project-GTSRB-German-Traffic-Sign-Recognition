...
The notebook reports final test metrics only after the training pipeline has been selected on validation data.
...
5. Train a small CNN (3 conv blocks + BatchNorm), select the checkpoint on validation macro-F1, then evaluate once on the official test set
...
- The checkpoint and all pipeline choices are selected on validation data; the official test set is reserved for final reporting.
